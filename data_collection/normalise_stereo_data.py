...
import numpy as np
from normalise_stereo_p import (rolling_75p, clip_max, outlier_indicies,
                                zero_point, q)
...
for i in range(len(data)):
    ...
    save_name = f'STE_{date.year}.{date.month:0>2}.{date.day:0>2}_' \
...
np.save(f"{percentile_dir}STEREO_normal_p", normal_p)
np.save(f"{percentile_dir}STEREO_normal_d", normal_d)
